...
func buildConfigContent(port int, nodeIPs []string, dbPort int) string {
	...
	b.WriteString("    # fall 2  = mark DOWN after 2 consecutive failures — detects dead primary in ~2s\n")
	b.WriteString("    # rise 2  = mark UP after 2 consecutive successes (avoid premature recovery)\n")
	b.WriteString("    # inter   = check every 1s when healthy\n")
	b.WriteString("    # fastinter = check every 200ms when just failed (detect recovery fast)\n")
	b.WriteString("    # downinter = check every 500ms when DOWN\n")
	b.WriteString("    # on-marked-down shutdown-sessions = kill connections immediately when server goes down\n")
	b.WriteString("    default-server inter 1s fastinter 200ms downinter 500ms fall 2 rise 2 on-marked-down shutdown-sessions on-marked-up shutdown-backup-sessions\n")
	b.WriteString("\n")
