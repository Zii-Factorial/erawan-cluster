...
| `/etc/mysqlrouter/prod/mysqlrouter.conf` | yes (if `bootstrap_router: true`) | `router_id` |
| `/etc/systemd/system/mysqlrouter-prod.service` | yes (if `bootstrap_router: true`) | no |
...
### `/etc/mysqlrouter/prod/mysqlrouter.conf` (when `bootstrap_router: true`)

Auto-generated by `mysqlrouter --bootstrap`. Not a template — written at deploy time by Router itself. The `router_id` increments per node (1 on node 1, 2 on node 2, 3 on node 3); all routing config is identical.
...
connect_timeout = 15
read_timeout = 30
dynamic_state = /etc/mysqlrouter/prod/data/state.json
...
Verify: `sudo cat /etc/mysqlrouter/prod/mysqlrouter.conf`

---

### `/etc/systemd/system/mysqlrouter-prod.service` (when `bootstrap_router: true`)

Polls for GR ONLINE (up to 90 s) before starting Router, and restarts Router automatically if it crashes.

```ini
[Unit]
Description=MySQL Router for prod
...
Status: `sudo systemctl status mysqlrouter-prod`  
Logs: `sudo journalctl -u mysqlrouter-prod -n 100`
...
sudo systemctl status mysql mysqlrouter-prod erawan-gr-watchdog.timer erawan-mysql-boot-recovery
...
# Check MySQL Router is routing correctly
mysql -h 127.0.0.1 -P 6446 -u clusteradmin -p -e "SELECT @@hostname, @@port;"
